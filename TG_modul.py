...
from aiogram.types import URLInputFile, InputMediaPhoto, BotCommand, BotCommandScopeDefault
...
async def time10sec(message: types.Message):
    while True:
        await message.answer(text='10 sec')
        await asyncio.sleep(10)


#Функции для старта и управления бота
@dp.message(Command("start"))
async def cmd_start(message: types.Message):
    kb = [
        [
            types.KeyboardButton(text="Начать отправку постов"),
            types.KeyboardButton(text="Остановить отправку постов"),
            types.KeyboardButton(text="Список групп")
         ]
    ]
    keyboard = types.ReplyKeyboardMarkup(keyboard=kb,resize_keyboard=True)
    await message.answer(text="Бот запущен", reply_markup=keyboard)

#Список групп из которых собираются посты
@dp.message(F.text == "Список групп")
async def start_send_posts(message: types.Message):
    name_group_list = "\n".join([f"{index}. https://vk.com/{word}" for index, word in enumerate(NAMES_GROUP_LIST, start=1)])
    await message.answer("Список групп\n" + name_group_list)

#Функции управления бота
@dp.message(F.text == "Начать отправку постов")
async def start_send_posts(message: types.Message):
    global sending_active
    sending_active = True
...
@dp.message(F.text == "Остановить отправку постов")
async def start_send_posts(message: types.Message):
    global sending_active
    sending_active = False
